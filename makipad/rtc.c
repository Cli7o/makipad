...
uint32_t convert_to_unixtime(rtc_time_t time) {
    ...
    unixtime = ((days * 24UL + time.hour) * 60 + time.minute) * 60 + time.second + SECONDS_FROM_1970_TO_2000 - (time.timezone * 3600);
    return unixtime;
}
...
char *rtc_read_time_str(void) {
    ...
    snprintf_nowarn(time_str, sizeof(time_str), "%02d:%02d:%02d%s", hour, rtc_time.minute, rtc_time.second, rtc_time.format == RTC_FORMAT_24H ? "" : (am_pm == RTC_AM ? "AM" : "PM"));
    return time_str;
}
...
char *rtc_read_date_time_iso8601_str(void) {
    ...
    snprintf_nowarn(date_time_str, sizeof(date_time_str), "%04d-%02d-%02dT%02d:%02d:%02d%s", rtc_time.year, rtc_time.month, rtc_time.date, hour, rtc_time.minute, rtc_time.second, rtc_time.format == RTC_FORMAT_24H ? "" : (am_pm == RTC_AM ? "AM" : "PM"));
    return date_time_str;
}
...
__attribute__((weak)) uint32_t get_fattime(void) {
    return (((uint32_t)rtc_time.year - 1980) << 25U) | ((uint32_t)rtc_time.month << 21U) | ((uint32_t)rtc_time.date << 16U) | ((uint32_t)rtc_time.hour << 11U) | ((uint32_t)rtc_time.minute << 5U) | ((uint32_t)rtc_time.second >> 1U);
}
